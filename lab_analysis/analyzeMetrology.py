...
plt.grid(alpha=0.5)
ax1.set_xlabel("Module Id", fontsize=16)
ax1.set_ylim(0, 1000)
ax1.set_xlim(0, 6)
ax1.set_ylabel(r"R ($\mu rad$)", fontsize=16)
...
plt.text(4.1+0.05, 800+6.5, "Tolerance on R", color="darkgreen", fontsize=16, alpha=0.5)

ax2 = ax1.twinx()
ax2.set_ylabel(r"Tx, Ty ($\mu m$)", fontsize=16)
ax2.set_ylim(0, 200)
...
plt.text(4.1+0.05, 50+2, "Tolerance on Tx", color="darkred", fontsize=16, alpha=0.5)
...
plt.text(4.1+0.05, 100+2, "Tolerance on Ty", color="darkblue", fontsize=16, alpha=0.5)

plt.legend(handles = [R_plot, Tx_plot, Ty_plot], labels=["R = rotation", r"Tx = translation $\perp$ to strips", r"Ty = translation $\parallel$ to strips"], loc="upper right", fontsize=16, bbox_to_anchor=(0.855, 1.34))
#plt.legend(handles = [R_plot, Tx_plot, Ty_plot], labels=["R = rotation", r"Tx = translation $\perp$ to strips", r"Ty = translation $\parallel$ to strips"], fontsize=16, loc="upper right")
#plt.tight_layout()
# plt.show()
